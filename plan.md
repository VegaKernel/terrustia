...
| Tier 2 | Underground cabins | `UndergroundHousesAndBuriedChests` | **≥400-700 estimated, corrected from ≥252** — most of the driving pass (`WorldGen.cs:17081-17240`) is buried chests this generator's own `structures::add_chest`/`biome_chest_loot` already cover; the actual "cabin" is a `CaveHouseBiome.Place(point, structures)` call into a whole undiscovered `Biome`-class room generator (`CaveHouseBiome.cs`, not yet read/measured), the same "Biome pattern" micro-biomes uses | `StructureMap` (confirmed — `GenVars.structures.CanPlace`/`AddProtectedStructure`, exactly as this generator's own `StructureMap`) | **Hard, corrected from Medium** — needs a whole `Biome`-class framework read first, not a plain transcription |
