...
| Tier 2 | Underground cabins | `UndergroundHousesAndBuriedChests` | **≥1,556, now fully measured, corrected from ≥252 then ≥400-700** — most of the driving pass (`WorldGen.cs:17081-17240`) is buried chests this generator's own `structures::add_chest`/`biome_chest_loot` already cover; the actual "cabin" is `CaveHouseBiome.Place` (92 lines) calling `HouseUtils.CreateBuilder` (286 lines, room-finding — see the DSL correction above) into `HouseBuilder` (911 lines: a real multi-room furniture engine — doors, tables, workbenches, pianos, bookcases, chairs, chests, platforms, beams) plus 7 small per-material subclasses (~247 lines total: Wood/Desert/Granite/Ice/Jungle/Marble/Mushroom, each just naming that material's tile/wall/furniture-style ids) | `StructureMap` (confirmed) **+ a real, narrow slice of `WorldUtils`/`Actions`/`Modifiers`/`Shapes`** (see the DSL correction above — not the full framework, but genuinely more than "none") | **Hard, confirmed** — on par with living trees and underworld ruins, not a mid-sized item; `HouseBuilder.cs` alone, the furniture-placement core, is not yet read in detail |
