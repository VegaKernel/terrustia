...
/// itself must be non-solid and cannot have a player-safe (`Main.wallHouse`) background wall. The
/// clearance rectangle starts one row above it, covers that column and the immediately-left column,
/// and is three tiles high. Solid tiles or lava inside the rectangle reject the candidate. Other
/// liquid kinds are handled by the later liquid-source check.
pub fn chosen_point_is_clear(world: &World, x: i32, chosen_y: i32) -> bool {
    ...
    if wall_house::safe(chosen.wall) {
        return false;
    }
...
mod tests {
    ...
    #[test]
    fn safe_wall_at_the_chosen_tile_rejects_the_candidate() {
        let mut world = candidate();
        let mut tile = Tile::AIR;
        tile.wall = 1; // Stone Wall: Main.wallHouse = true.
        assert!(world.set_tile(50, 40, tile));
        assert!(!chosen_point_is_clear(&world, 50, 40));
    }

    #[test]
    fn natural_unsafe_wall_does_not_reject_the_candidate() {
        let mut world = candidate();
        let mut tile = Tile::AIR;
